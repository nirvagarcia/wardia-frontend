...
import { AddBoardModal } from "./board/modals/add-board-modal";
import { ConfirmModal } from "@/shared/components/modals/confirm-modal";
import Image from "next/image";
import { cn } from "@/shared/utils/cn";

function SortableBoardCard({
  board, onEdit, onDelete, onArchive, onClone, onShare, onOpen,
}: {
  board: IBoardSummary;
  onEdit: (b: IBoardSummary) => void;
  onDelete: (b: IBoardSummary) => void;
  onArchive: (b: IBoardSummary) => void;
  onClone: (b: IBoardSummary) => void;
  onShare: (b: IBoardSummary) => void;
  onOpen: (b: IBoardSummary) => void;
}) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes}>
      <motion.div
        layout
        className={cn(
          "group relative bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200/80 dark:border-zinc-800/80",
          "hover:border-cyan-300 dark:hover:border-cyan-700 hover:shadow-lg transition-all duration-200 overflow-hidden cursor-pointer"
        )}
        onClick={() => onOpen(board)}
      >
        <div className="relative h-28 overflow-hidden">
          {board.coverImage ? (
            <Image src={board.coverImage} alt={board.title} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 25vw" className="object-cover" />
          ) : (
            <div className="w-full h-full bg-gradient-to-br from-cyan-400/20 via-teal-400/10 to-zinc-100 dark:from-cyan-900/30 dark:via-teal-900/10 dark:to-zinc-800" />
          )}
          <div
            {...listeners}
            onClick={(e) => e.stopPropagation()}
            className="absolute top-2 left-2 w-6 h-6 flex items-center justify-center rounded-md bg-white/70 dark:bg-zinc-900/70 opacity-0 group-hover:opacity-100 transition-opacity cursor-grab active:cursor-grabbing"
          >
          ...
          <div className="absolute top-2 right-2" onClick={(e) => e.stopPropagation()}>
            <button
              ...
              className="w-7 h-7 flex items-center justify-center rounded-lg bg-white/80 dark:bg-zinc-900/80 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white dark:hover:bg-zinc-800"
            >
          ...
          {board.shareToken && (
            <div className="absolute bottom-2 right-2 flex items-center gap-1 px-1.5 py-0.5 rounded-md bg-cyan-500/90 text-white text-xs">
        ...
        <div className="p-4">
          ...
          {board.description && (
            <p className="text-xs text-zinc-500 dark:text-zinc-400 truncate mb-3">{board.description}</p>
          )}
          ...
          <div className="flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400 mb-2">
            ...
            {stats.estimatedTotal > 0 && (
              <span className="font-medium text-zinc-700 dark:text-zinc-300">
                {formatPrice(stats.estimatedTotal, "PEN")}
              </span>
            )}
          </div>
...
function BoardMenu({
  board, onClose, onEdit, onDelete, onArchive, onClone, onShare,
}: {
  board: IBoardSummary;
  onClose: () => void;
  onEdit: (b: IBoardSummary) => void;
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 max-w-sm w-full shadow-2xl">
        ...
        <div className="flex gap-2">
          <input
            readOnly
            value={shareUrl}
            className="flex-1 text-xs px-3 py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 truncate"
          />
          <button
            onClick={copy}
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-cyan-500 hover:bg-cyan-600 text-white text-xs font-medium transition-colors"
          >
            <LinkIcon className="w-3.5 h-3.5" />
            {copied ? "¡Copiado!" : "Copiar"}
          </button>
        </div>
        <button onClick={onClose} className="mt-4 w-full py-2 rounded-lg border border-zinc-200 dark:border-zinc-700 text-sm text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
          Cerrar
        </button>
      </div>
    </div>
...
export function PlanningView() {
  ...
  useEffect(() => {
    if (storageData && storageData.usedPercent >= 80) {
      toast.warning(`⚠️ Espacio de imágenes casi lleno: ${storageData.usedMB}/${storageData.totalMB} MB`, {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => router.push("/planning/archived")}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors"
          >
              ...
              <AnimatePresence>
                {boards.map((board) => (
                  <SortableBoardCard
                    ...
                    onArchive={(b) => {
                      archiveBoard.mutate(b.id, {
                        onSuccess: () => toast.success("Tablero archivado"),
                        onError: () => toast.error("Error al archivar"),
                      });
                    }}
                    onClone={(b) => {
                      cloneBoard.mutate(b.id, {
                        onSuccess: () => toast.success("Tablero clonado"),
      ...
      <AddBoardModal
        isOpen={isAddOpen || !!editingBoard}
        editingBoard={editingBoard ?? undefined}
        onClose={() => { setIsAddOpen(false); setEditingBoard(null); }}
        onSave={async (data) => {
          try {
            if (editingBoard) {
            ...
            toast.success(editingBoard ? "Tablero actualizado" : "Tablero creado");
          } catch {
      ...
      {deletingBoard && (
        <ConfirmModal
          isOpen
          title={t("planning.deleteBoard")}
          message={t("planning.deleteConfirmBoard").replace("{title}", deletingBoard.title)}
