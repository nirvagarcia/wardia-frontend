...
export function SharedBoardView({ shareToken }: SharedBoardViewProps) {
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <div className="bg-white dark:bg-zinc-950 border-b border-zinc-200 dark:border-zinc-800 px-4 py-3">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <span className="text-xs px-2.5 py-1 rounded-full bg-cyan-100 dark:bg-cyan-950/50 text-cyan-700 dark:text-cyan-300 border border-cyan-200 dark:border-cyan-800">
            Vista compartida
          </span>
        </div>
      </div>
                    ...
                    <div key={item.id} className={cn("bg-zinc-50 dark:bg-zinc-800 rounded-xl border overflow-hidden", isPurchased ? "border-emerald-200 dark:border-emerald-800/60 opacity-70" : "border-zinc-200 dark:border-zinc-700")}>
                      ...
                      <div className="p-2.5">
                        <div className="flex items-start gap-1.5">
                          ...
                          <p className={cn("text-xs font-medium line-clamp-2 leading-snug", isPurchased ? "text-zinc-400 line-through" : "text-zinc-800 dark:text-zinc-200")}>
                            {item.title}
                          </p>
                        </div>
                        {item.priceValue != null && (
                          <p className="text-xs font-semibold text-cyan-600 dark:text-cyan-400 mt-1 ml-3">{formatPrice(item.priceValue, item.priceCurrency)}</p>
                        )}
                        <div className="flex items-center justify-between mt-2 ml-3">
                          <span className={cn("text-xs", isPurchased ? "text-emerald-500" : "text-zinc-400")}>
                            {isPurchased ? "✓ Comprado" : "Pendiente"}
                          </span>
                          {item.links.length > 0 && (
                            <a href={item.links[0]?.url} target="_blank" rel="noopener noreferrer" className="text-zinc-400 hover:text-cyan-500 transition-colors">
