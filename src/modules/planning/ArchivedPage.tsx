    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-zinc-500">{t("planning.noArchived")}</p>
        </div>
      ) : (
              ...
              <div key={board.id} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden opacity-80 hover:opacity-100 transition-opacity">
                {board.coverImage && (
                  <div className="relative h-20 overflow-hidden">
                    <Image src={board.coverImage} alt={board.title} fill sizes="(max-width:640px) 100vw,(max-width:1024px) 50vw,33vw" className="object-cover grayscale-[40%]" />
                  </div>
                )}
                <div className="p-4">
                  <h3 className="font-semibold text-zinc-700 dark:text-zinc-300 truncate mb-0.5">{board.title}</h3>
                  <p className="text-xs text-zinc-400 mb-3">{stats.totalItems} items • {stats.purchasedItems} {t("planning.purchased")}</p>
                  {stats.estimatedTotal > 0 && <p className="text-xs text-zinc-400 mb-3">{formatPrice(stats.estimatedTotal, "PEN")} {t("planning.estimatedTotal").toLowerCase()}</p>}
                  <div className="flex gap-2">
                    <button onClick={() => archiveBoard.mutate(board.id, { onSuccess: () => toast.success("Tablero restaurado"), onError: () => toast.error("Error al restaurar") })}
                      className={cn("flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-medium transition-colors", "border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800")}>
                      <RotateCcw className="w-3.5 h-3.5" />{t("planning.restoreBoard")}
                    </button>
                    <button onClick={() => setDeletingBoard(board)} className="flex items-center justify-center px-3 py-2 rounded-xl border border-rose-200 dark:border-rose-800/50 text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 transition-colors">
