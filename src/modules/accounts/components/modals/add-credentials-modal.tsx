...
const CREDENTIAL_ICONS: Record<CredentialType, React.ElementType> = { bank: Building2, debit: Wallet, credit: CreditCard };
const CREDENTIAL_TYPES: CredentialType[] = ["bank", "debit", "credit"];

function TypeSelector({ onSelect, t }: { onSelect: (type: CredentialType) => void; t: (key: string) => string }) {
  return (
    <div className="p-6 space-y-3">
      <p className="text-sm text-zinc-500 dark:text-zinc-400 mb-4">{t("credentials.selectType")}</p>
      {CREDENTIAL_TYPES.map((type) => {
        ...
        return (
          <button key={type} type="button" onClick={() => onSelect(type)}
            ...
            <div>
              <p className="font-medium text-zinc-900 dark:text-white text-sm">{t(`credentials.type${cap}`)}</p>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{t(`credentials.type${cap}Desc`)}</p>
            </div>
          </button>
        );
      })}
...
function SubmitButton({ isSubmitting, t }: { isSubmitting: boolean; t: (key: string) => string }) {
  return (
    <button type="submit" disabled={isSubmitting}
      ...
      {isSubmitting ? t("credentials.saving") : t("credentials.save")}
    </button>
  );
}

function BankForm({ initial, onSubmit, isSubmitting, t }: {
  initial?: Partial<BankCredentialModalData>; onSubmit: (d: BankCredentialModalData) => void; isSubmitting: boolean; t: (key: string) => string;
}) {
  ...
  return (
    <form id="credential-form" onSubmit={handleSubmit} className="p-6 space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <FormField label={t("credentials.bankName") + " *"} error={errors['bankName']}><Input value={form.bankName} onChange={(e) => set("bankName", e.target.value)} placeholder="BBVA, BCP..." className={cn(errors['bankName'] && "border-red-500")} /></FormField>
        <FormField label={t("credentials.credentialName")} error={errors['credentialName']}><Input value={form.credentialName ?? ""} onChange={(e) => set("credentialName", e.target.value)} placeholder="Cuenta principal" /></FormField>
      </div>
      <FormField label={t("credentials.description")} error={errors['description']}><Input value={form.description ?? ""} onChange={(e) => set("description", e.target.value)} placeholder="Opcional" /></FormField>
      <FormField label={t("credentials.username") + " *"} error={errors['username']}><Input value={form.username} onChange={(e) => set("username", e.target.value)} placeholder="usuario@banco.com" className={cn("font-mono", errors['username'] && "border-red-500")} autoComplete="off" /></FormField>
      <FormField label={t("credentials.password") + " *"} error={errors['password']}><PasswordInput value={form.password} onChange={(v) => set("password", v)} error={errors['password']} /></FormField>
      <SubmitButton isSubmitting={isSubmitting} t={t} />
    </form>
  );
}

function DebitForm({ initial, onSubmit, isSubmitting, t }: {
  initial?: Partial<DebitCardModalData>; onSubmit: (d: DebitCardModalData) => void; isSubmitting: boolean; t: (key: string) => string;
}) {
  ...
  return (
    <form id="credential-form" onSubmit={handleSubmit} className="p-6 space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <FormField label={t("credentials.bankName") + " *"} error={errors['bankName']}><Input value={form.bankName} onChange={(e) => set("bankName", e.target.value)} placeholder="BBVA, BCP..." className={cn(errors['bankName'] && "border-red-500")} /></FormField>
        <FormField label={t("credentials.cardName") + " *"} error={errors['cardName']}><Input value={form.cardName} onChange={(e) => set("cardName", e.target.value)} placeholder="Debito BBVA" className={cn(errors['cardName'] && "border-red-500")} /></FormField>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <FormField label={t("credentials.accountType") + " *"} error={errors['accountType']}>
          <Select value={form.accountType} onValueChange={(v) => set("accountType", v as DebitCardModalData["accountType"])}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>
              <SelectItem value="savings">{t("credentials.savings")}</SelectItem>
              <SelectItem value="checking">{t("credentials.checking")}</SelectItem>
            </SelectContent>
          </Select>
        </FormField>
        <FormField label={t("credentials.cardNetwork") + " *"} error={errors['cardNetwork']}>
      ...
      <FormField label={t("credentials.accountNumber") + " *"} error={errors['accountNumber']}><Input value={form.accountNumber} onChange={(e) => set("accountNumber", e.target.value)} placeholder="12345678901" className={cn("font-mono", errors['accountNumber'] && "border-red-500")} /></FormField>
      <FormField label={t("credentials.cci")}><Input value={form.cci ?? ""} onChange={(e) => set("cci", e.target.value)} placeholder="00212345678901234567" className="font-mono" /></FormField>
      <div className="grid grid-cols-3 gap-4">
        <FormField label={t("credentials.expiryMonth") + " *"} error={errors['expiryMonth']}>
        ...
        <FormField label={t("credentials.expiryYear") + " *"} error={errors['expiryYear']}>
        ...
        <FormField label={t("credentials.cvv") + " *"} error={errors['cvv']}><PasswordInput value={form.cvv} onChange={(v) => set("cvv", v)} placeholder="***" error={errors['cvv']} /></FormField>
      </div>
      <SubmitButton isSubmitting={isSubmitting} t={t} />
    </form>
  );
...
function CreditForm({ initial, onSubmit, isSubmitting, t }: {
  initial?: Partial<CreditCardModalData>; onSubmit: (d: CreditCardModalData) => void; isSubmitting: boolean; t: (key: string) => string;
}) {
  ...
  return (
    <form id="credential-form" onSubmit={handleSubmit} className="p-6 space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <FormField label={t("credentials.bankName") + " *"} error={errors['bankName']}><Input value={form.bankName} onChange={(e) => set("bankName", e.target.value)} placeholder="BBVA, Falabella..." className={cn(errors['bankName'] && "border-red-500")} /></FormField>
        <FormField label={t("credentials.cardName") + " *"} error={errors['cardName']}><Input value={form.cardName} onChange={(e) => set("cardName", e.target.value)} placeholder="Visa Gold BBVA" className={cn(errors['cardName'] && "border-red-500")} /></FormField>
      </div>
      <FormField label={t("credentials.cardHolder") + " *"} error={errors['cardholderName']}><Input value={form.cardholderName} onChange={(e) => set("cardholderName", e.target.value)} placeholder="JUAN PEREZ LOPEZ" className={cn("font-mono uppercase", errors['cardholderName'] && "border-red-500")} /></FormField>
      <div className="grid grid-cols-2 gap-4">
        <FormField label={t("credentials.cardNetwork") + " *"} error={errors['cardNetwork']}>
        ...
        <FormField label={t("credentials.cutoffDay") + " *"} error={errors['cutoffDay']}><Input type="number" min={1} max={31} value={form.cutoffDay} onChange={(e) => set("cutoffDay", Number(e.target.value))} className={cn(errors['cutoffDay'] && "border-red-500")} /></FormField>
      </div>
      <div className="grid grid-cols-2 gap-4">
        <FormField label={t("credentials.creditLimit") + " *"} error={errors['creditLimitValue']}><Input type="number" min={0} step="0.01" value={form.creditLimitValue} onChange={(e) => set("creditLimitValue", Number(e.target.value))} className={cn("font-mono", errors['creditLimitValue'] && "border-red-500")} /></FormField>
        <FormField label={t("credentials.currency") + " *"} error={errors['creditLimitCurrency']}>
      ...
      <FormField label={t("credentials.accountNumber")}><Input value={form.accountNumber ?? ""} onChange={(e) => set("accountNumber", e.target.value)} placeholder="Opcional" className="font-mono" /></FormField>
      <FormField label={t("credentials.cci")}><Input value={form.cci ?? ""} onChange={(e) => set("cci", e.target.value)} placeholder="Opcional" className="font-mono" /></FormField>
      <div className="grid grid-cols-3 gap-4">
        <FormField label={t("credentials.expiryMonth") + " *"} error={errors['expiryMonth']}>
          <Select value={String(form.expiryMonth)} onValueChange={(v) => set("expiryMonth", Number(v))}>
            <SelectTrigger><SelectValue /></SelectTrigger>
...
export function AddCredentialsModal({ isOpen, onClose, onSave, onUpdate, editingCredential }: AddCredentialsModalProps): React.JSX.Element | null {
  ...
  const TYPE_CAPS: Record<CredentialType, string> = { bank: "Bank", debit: "Debit", credit: "Credit" };
  const title = isEditing
    ? t(`credentials.edit${TYPE_CAPS[editingCredential.type]}`)
    : step === "select"
    ? t("credentials.addCredential")
    : t(`credentials.new${TYPE_CAPS[step as CredentialType]}`);
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-3 sm:p-4" onClick={onClose}>
      <div className="bg-white dark:bg-zinc-900 rounded-2xl w-full max-w-lg max-h-[92vh] flex flex-col animate-slide-up" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-y-auto">
          {step === "select" && <TypeSelector onSelect={(type) => setStep(type)} t={t} />}
          {step === "bank" && <BankForm initial={isEditing ? editingCredential : undefined} onSubmit={handleBankSubmit} isSubmitting={isSubmitting} t={t} />}
          {step === "debit" && <DebitForm initial={isEditing ? (editingCredential as Partial<DebitCardModalData>) : undefined} onSubmit={handleDebitSubmit} isSubmitting={isSubmitting} t={t} />}
          {step === "credit" && <CreditForm initial={isEditing ? (editingCredential as Partial<CreditCardModalData>) : undefined} onSubmit={handleCreditSubmit} isSubmitting={isSubmitting} t={t} />}
        </div>
      </div>
    </div>
  );
