...
import {
  Receipt, Plus, Filter, History, Layers, TrendingUp, TrendingDown, BarChart2, ChevronDown, Banknote,
} from "lucide-react";
  ...
  if (isError) return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center space-y-4">
        ...
        <button onClick={() => window.location.reload()} className="px-4 py-2 bg-cyan-500 text-white rounded-lg hover:bg-cyan-600">{t("common.back")}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <header className="space-y-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2 shrink-0 mt-1">
            <button onClick={() => setIsStartPeriodModalOpen(true)}
            ...
            <div ref={menuRef} className="relative">
              <button onClick={() => setIsMenuOpen((prev) => !prev)}
                ...
                <ChevronDown className={cn("w-3.5 h-3.5 transition-transform duration-200", isMenuOpen && "-rotate-180")} />
              </button>
              {isMenuOpen && (
                <div className="absolute right-0 top-full mt-1.5 w-48 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-lg overflow-hidden z-20">
                  <button onClick={() => { setIsMenuOpen(false); router.push("/transactions/insights"); }}
                  ...
                  <button onClick={() => { setIsMenuOpen(false); router.push("/transactions/history"); }}
                    className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
      ...
      <TransactionStats
        ...
        incomeLabel={t("transactions.income")} expensesLabel={t("transactions.expenses")}
        balanceLabel={t("transactions.balance")} thisMonthLabel={periodLabel}
      ...
      <div className="flex gap-2 justify-end">
        <button onClick={() => setShowFilters(!showFilters)}
          className={cn("px-3 md:px-4 py-1.5 md:py-2 rounded-xl text-xs md:text-sm font-medium transition-colors flex items-center gap-2",
            showFilters || hasActiveFilters ? "bg-cyan-500 text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-gray-300 hover:bg-zinc-200 dark:hover:bg-zinc-700")}>
      ...
      {showFilters && (
        <div className="bg-white dark:bg-zinc-900/50 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 space-y-4">
          <div>
            <h3 className="font-semibold text-zinc-900 dark:text-white mb-3">{t("transactions.filters")}</h3>
                ...
                return (
                  <button key={type} onClick={() => setFilterType(type)}
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      <span className={cn("text-sm font-medium truncate", isActive ? activeText : "text-zinc-600 dark:text-zinc-400")}>{label}</span>
                    </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              ...
              {hasActiveFilters && <button onClick={clearFilters} className="text-sm text-cyan-500 hover:text-cyan-600 font-medium">{t("transactions.clearFilters")}</button>}
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {currentFilterCategories.map((category) => {
                const isActive = selectedCategories.includes(category.key);
                const categoryCount = allTransactions.filter((tr) => tr.category === category.key).length;
                return (
                  <button key={category.key} onClick={() => toggleCategory(category.key)}
                    className={cn("flex items-center justify-between gap-2 p-4 rounded-xl border-2 transition-all",
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("text-sm font-medium", isActive ? "text-cyan-600 dark:text-cyan-400" : "text-zinc-600 dark:text-zinc-400")}>{t(category.labelKey)}</span>
                    </div>
                    <span className={cn("text-xs font-bold px-1.5 py-0.5 rounded-full", isActive ? "bg-cyan-500 text-white" : "bg-zinc-100 dark:bg-zinc-700 text-zinc-500 dark:text-zinc-400")}>{categoryCount}</span>
                  </button>
                );
              })}
            </div>
            ...
            <TransactionCard key={transaction.id} transaction={transaction}
              {...(!transaction.isService && {
                ...
                onDelete: (id) => {
                  ...
                  setConfirmDelete({ isOpen: true, id, description: found?.description ?? "" });
                },
              })}
            />
          ))
      ...
      <AddTransactionModal isOpen={isAddModalOpen} onClose={handleCloseModal}
        onAdd={handleAddTransaction} editingTransaction={editingTransaction} onUpdate={handleUpdateTransaction} />
      ...
      <StartPeriodModal isOpen={isStartPeriodModalOpen} onClose={() => setIsStartPeriodModalOpen(false)} currentPeriodLabel={periodLabel} />
    </div>
  );
}
