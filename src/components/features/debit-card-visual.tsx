...
const getBankGradient = (bankName: string): string => {
  ...
  if (bank.includes("bcp") || bank.includes("crédito del perú")) {
    return "from-blue-600 via-blue-700 to-blue-800"; // BCP Azul
  } else if (bank.includes("interbank")) {
    return "from-emerald-600 via-emerald-700 to-green-700"; // Interbank Verde
  } else if (bank.includes("bbva")) {
    return "from-blue-700 via-blue-800 to-indigo-900"; // BBVA Azul oscuro
  } else if (bank.includes("scotiabank")) {
    return "from-red-600 via-red-700 to-red-800"; // Scotiabank Rojo
  }
  return "from-slate-600 via-slate-700 to-slate-800"; // Default gris
};
